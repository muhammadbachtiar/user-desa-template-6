                            ...
                            <li key={item.id} className="group border-b border-gray-50 dark:border-gray-800/30 last:border-none last:pb-0">
                                <Link href={`/article/${item.slug}`} className="flex gap-4 items-start">
                                    ...
                                    <div className="flex-1 min-w-0 flex flex-col justify-between self-stretch">
                                        <h4 className="text-xs sm:text-sm font-bold text-neutral-text group-hover:text-brand-primary transition-colors leading-snug line-clamp-2" title={item.title}>
                                            {item.title}
                                        </h4>
                                        ...
                                        <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[10px] text-neutral-text-muted mt-1.5 font-semibold min-w-0">
                                            <span className="text-brand-primary font-bold uppercase tracking-wider shrink-0" title={rawCategory}>
                                                {truncateWords(rawCategory, 2)}
                                            </span>
                                            <span className="text-gray-300 select-none shrink-0">•</span>
                                            <span className="shrink-0">{formatDate(item.published_at || item.created_at)}</span>
                                        </div>
                                    </div>
